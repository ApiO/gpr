...
I32 full(gpr_hash_t *h)
{
  return h->num_values >= (I32)(gpr_array_size(h->buckets)*MAX_LOAD_FACTOR);
}

void find_index(gpr_hash_t *h, U64 key, find_result_t *fr)
{
...
U32 make_index(gpr_hash_t *h, U64 key, U32 value_pos)
{
  find_result_t fr;
  index_t       index;
...
U32 find_or_make_index(gpr_hash_t *h, U64 key)
{
  find_result_t fr;
  find_index(h, key, &fr);
...
void erase(gpr_hash_t *h, const U32 s, find_result_t *fr)
{
...
void rehash(gpr_hash_t *h, const U32 s, U32 size)
{
  U32 i;
...
void grow(gpr_hash_t *h, const U32 s)
{
